...
class GraphBuilder:
    ...
    def extract_node_features(self, time_window=None, feature_cols=None):
        ...
        # Get unique locations
        if self.node_coords is None:
            self.extract_node_coordinates(df)
        ...
        for coord in self.node_coords:
            lat, lon = coord
            matches = df[
                (np.abs(df['latitude'] - lat) < 0.001) &
                (np.abs(df['longitude'] - lon) < 0.001)
            ]
            
            if len(matches) > 0:
                # Use mean if multiple matches
                features = matches[feature_cols].mean().values
            else:
                # Use zeros if no match
